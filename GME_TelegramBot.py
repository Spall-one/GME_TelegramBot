...
async def istruzioni(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg += (
        ...
        f"🎯 OBIETTIVO"
        f"Ogni giorno si scommette sulla variazione percentuale del titolo GME (chiusura odierna rispetto alla chiusura di ieri)\n."
        f"Ogni utente ha un saldo in € virtuali che sale o scende in base ai risultati.\n"
        f"La classifica rappresenta i debiti o crediti che gli utenti dovranno pagare/ricevere il CRISTO DIO DI GIORNO IN CUI AVVERRA' IL MOASS\n"
        f"(Perchè avverrà, vero?)\n\n"

        f"────────────────────\n"
        f"📌 COME FARE UNA SCOMMESSA\n"
        f"────────────────────\n"
        f"Usa il comando:\n"
        ...
        f"Esempi:\n"
        f"• /bet 1.5      → prevedi +1,50%\n"
        f"• /bet -0.75    → prevedi -0,75%\n"
        f"• /bet 0        → prevedi 0,00%\n"

        f"Regole:\n"
        f"• Puoi usare solo numeri decimali con il punto (es. 0.5, -1.23).\n"
        f"• La percentuale è riferita alla variazione giornaliera di GME.\n"
        f"• Puoi scommettere SOLO UNA VOLTA al giorno, evita pagliacciate.\n"
        f"• Non puoi scommettere se sei bannato.E se sei bannato vuol dire che sei un cagacazzo\n"
        f"────────────────────\n"
        f"⏰ ORARI & GIORNI DI GIOCO\n"
        f"────────────────────\n"
        f"• Si gioca solo nei giorni di mercato aperto (no weekend, no giorni in CHIUSURE_MERCATO).\n"
        f"• Le scommesse si possono fare dalle 00:00 alle 15:30. Non cagate il cazzo per i giorni in cui c'è il cambio dell'ora sfasato con gli USA.\n"
        ...
        f"il bot:\n"
        f"1️- Recupera la variazione giornaliera di GME per quella data.\n"
        f"2️- Considera tutte le scommesse registrate per quel giorno.\n"
        f"3️- Ordina i giocatori in base alla distanza assoluta dal valore reale (Diff).\n"

        f"Premi Fissi:\n"
        f"• 1° classificato: +150€\n"
        f"• 2° classificato: +100€\n"
        f"• 3° classificato: +50€\n"
        f"• Ultimi  3: -50€, -100€, -150€ (dal terzultimo all’ultimo)\n"

        f"Variabile:\n"
        f"• Il bot calcola delle differenze tra i risultati dei vari giocatori e le trasforma in vincite/perdite .\n"
        f"• Parte variabile: il bot confronta quanto ogni previsione è lontana dal valore reale.\n"
        f"Ogni giocatore nella prima metà della classifica giornaliera guadagna quello che perde il corrispettivo giocatore nella seconda metà della classifica giornaliera\n"
        f"La formula utilizzata è:differenza_di_errore_in_punti_percentuali × 5€.\n"
        f"In pratica, ogni 0,1 punti percentuali di errore in più tra due giocatori valgono 0,5€ a favore del più preciso (e -0,5€ per l’altro).\n"

        f"Se hai scommesso:\n"
        f"• Puoi vincere o perdere in base a posizione, fissi e variabili.\n"

        f"Se NON hai scommesso:\n"
        f"• Perdi sempre 10€ di penalità giornaliera.\n"
        f"• I 10€ per ogni inattivo vanno a formare il “tesoretto” della settimana.\n"

        f"────────────────────\n"
        f"🎯 PERFECT GUESS\n"
        f"────────────────────\n"
        f"Se almeno un giocatore indovina esattamente la variazione (es. GME fa +0.00% e qualcuno ha scommesso 0.00%), scatta la modalità “perfect guess”:\n"

        f"• Il perfect guesser si porta a casa tutto il bottino: prende un fisso di 300€.\n"
        f"• Viene calcolata comunque la parte variabile contro i giocatori perdenti, quindi quelli della seocnda metà della classifica giornaliera.\n"
       

        f"────────────────────\n"
        f"💰 TESORETTO SETTIMANALE\n"
        f"────────────────────\n"
        f"• Ogni giorno, chi NON scommette perde 10€.\n"
        f"• Questi 10€ a testa vengono accumulati in un tesoretto settimanale.\n"
        f"• Il comando /vincitore aggiorna il tesoretto e il saldo dei non scommettitori.\n"

        f"Assegnazione tesoretto:\n"
        f"• Il venerdì è il grande giorno! Chi arriva primo il venerdì si porta a casa il tesoretto della settimana!"
        
        f"────────────────────\n"
        f"📊 PRINCIPALI COMANDI\n"
        f"────────────────────\n"
        f"/bet <valore>\n"
          f"Registra la tua scommessa giornaliera (in %).\n"

        f"/vincitore\n"
          f"Calcola e mostra i risultati del giorno corrente (se non è già stato calcolato).\n"

        f"/vincitore yesterday\n"
          f"Stessa cosa ma riferita a ieri:\n"
          f"– usa la data di ieri,\n"
          f"– aggiorna saldi e tesoretto di quella data,\n"
          f"– non ricalcola se esiste già un record in winners per quella data.\n"

        f"/scommesse\n"
          f"Mostra le scommesse registrate per la giornata in corso.\n"

        f"/classifica\n"
          f"Mostra la classifica completa con i saldi correnti.\n"

        f"/bilancio\n"
          f"Mostra il tuo saldo personale.\n"

        f"/istruzioni\n"
          f"Mostra questo manuale.\n"

        f"/ban <username> <giorni>   (solo admin)\n"
          f"Banna un utente per un certo numero di giorni. Durante il ban non può scommettere.\n"

        f"/unban <username>          (solo admin)\n"
          f"Sblocca un utente bannato.\n"

        f"/bannati                    (solo admin)\n"
          f"Mostra la lista degli utenti attualmente bannati.\n"
